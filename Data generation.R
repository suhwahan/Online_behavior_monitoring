### Calculate item response function --------
...
### Calculate response time density -------
...
genLNRTs <- function (alp, bet, tau){
  ...
  for (j in 1:Nexaminee){
    RT[j,] <- rlnorm(Nitem, bet-tau[j], 1/alp) # pdf function;
  }
  return(list(RT=RT))
}

### Null data generation function--------
# Note. The transformation of the parmameters part of this function came from Dr. Hyeon-Ah Kang (hkang@austin.utexas.edu)
nullDataGen <- function(nitem, nexaminee, rho_item, rho_person, c_f, 
                        mu_a, var_a, mu_alp, var_alp){
  
  ### Hyperparameters ---
  
  # Note. item parameters are assumed to follow a multivariate normal distribution (Kang, 2017, 2020)
...
genResp3PL_prob_th <- function(a, b, c, theta, D = 1.702, prob_th){ 
  # a, b, c, theta as vectors 
  # prob_threshold
  Nexaminee <- length(theta)
  Nitem <- length(a)
